...
from sqlalchemy import Boolean, DateTime, ForeignKey, Numeric, String, Text, Index, JSON
...
class Product(Base):
    ...
    gender: Mapped[str | None] = mapped_column(String(20), nullable=True, default="Women", index=True)
    size_chart: Mapped[dict | None] = mapped_column(JSON, nullable=True)
    description: Mapped[str | None] = mapped_column(Text, nullable=True)
    rating_average: Mapped[Decimal] = mapped_column(Numeric(3, 2), nullable=False, default=Decimal("4.8"))
    rating_count: Mapped[int] = mapped_column(nullable=False, default=15)
